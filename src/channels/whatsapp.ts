...
function installLibsignalConsoleRedaction(): void {
  if (libsignalConsoleRedactionInstalled) return;
  ...
  console.info = (...args: unknown[]) => {
    if (isSensitiveLibsignalLog(args)) {
      log.debug('Suppressed libsignal session log', { message: String(args[0] ?? '') });
      return;
    }
    originalInfo(...args);
  };
...
function isSensitiveLibsignalLog(args: unknown[]): boolean {
  const first = typeof args[0] === 'string' ? args[0] : '';
  if (!/^(Closing session:|Opening session:|Removing old closed session:|Session already closed)/.test(first)) {
    return false;
  }
  return args.some((arg) => containsSignalSessionKey(arg));
}

function containsSignalSessionKey(value: unknown, depth = 0): boolean {
  if (!value || typeof value !== 'object' || depth > 2) return false;
  const obj = value as Record<string, unknown>;
  const indexInfo = obj.indexInfo;
  if (indexInfo && typeof indexInfo === 'object' && 'remoteIdentityKey' in indexInfo) return true;
  return Object.values(obj).some((child) => containsSignalSessionKey(child, depth + 1));
}

installLibsignalConsoleRedaction();

// Pairing-code auth needs Baileys' platform id helper to emit enum values,
// not character codes. The standalone setup helper applies the same patch;
// keeping it here makes service-side re-pairing use the working path too.
const require = createRequire(import.meta.url);
try {
  const generics = require('@whiskeysockets/baileys/lib/Utils/generics') as Record<string, unknown>;
  ...
  // QR auth still works if this CJS compatibility patch cannot be applied.
}
...
const DISTRIBUTED_COGNITION_REPLY_TAG = 'DC:';
...
function shouldTagDistributedCognitionReply(): boolean {
  return ASSISTANT_NAME.trim().toLowerCase() === 'distributed cognition';
}

function tagWhatsAppReply(text: string): string {
  if (!shouldTagDistributedCognitionReply()) return text;
  const trimmed = text.trimStart();
  if (/^DC\s*[:：-]/i.test(trimmed)) return trimmed;
  return `${DISTRIBUTED_COGNITION_REPLY_TAG} ${trimmed}`;
}

function formatOutgoingWhatsAppText(text: string): string {
  const formatted = tagWhatsAppReply(formatWhatsApp(text));
  if (shouldTagDistributedCognitionReply() || ASSISTANT_HAS_OWN_NUMBER) return formatted;
  return `${ASSISTANT_NAME}: ${formatted}`;
}
...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    const privateSafety = loadWhatsAppPrivateModeConfig(env);
    const fromMeChatAllowlist = new Set(
      (env.WHATSAPP_FROM_ME_CHATS ?? '')
        .split(',')
        .map((jid) => jid.trim())
        .filter(Boolean),
    );
    ...
    if (privateSafety.enabled) {
      log.info('WhatsApp private mode enabled', { allowedJidConfigured: Boolean(privateSafety.allowedJid) });
    }
    ...
    const qrPngFile = path.join(DATA_DIR, 'whatsapp-qr.png');
    const qrTextFile = path.join(DATA_DIR, 'whatsapp-qr.txt');
    ...
    async function syncGroupMetadata(force = false): Promise<void> {
      if (privateSafety.enabled) {
        log.debug('Skipping WhatsApp group metadata sync in private mode');
        return;
      }
    ...
    function logSafetyBlock(
      phase: 'inbound' | 'outbound' | 'typing',
      reason: WhatsAppSafetyError['reason'],
      ids: { remoteJid?: string; chatJid?: string; senderJid?: string; targetJid?: string },
    ): void {
      log.warn('WhatsApp private-mode safety blocked message', whatsappSafetyLogFields(phase, reason, ids));
    }

    async function guardedSendMessage<TPayload>(
      jid: string,
      payload: TPayload,
    ): Promise<Awaited<ReturnType<WASocket['sendMessage']>>> {
      try {
        return await safeSendWhatsAppMessage(
          privateSafety,
          (to, body) => sock.sendMessage(to, body as never),
          jid,
          payload,
        );
      } catch (err) {
        if (err instanceof WhatsAppSafetyError) {
          logSafetyBlock('outbound', err.reason, { targetJid: jid });
        }
        throw err;
      }
    }
    ...
    async function flushOutgoingQueue(): Promise<void> {
      ...
      try {
        ...
        while (outgoingQueue.length > 0) {
          ...
          const sent = await guardedSendMessage(item.jid, { text: item.text });
    ...
    /** Download media from an inbound message for session-manager inbox staging. */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async function downloadInboundMedia(
      msg: WAMessage,
      normalized: any,
    ): Promise<Array<{ type: string; name: string; data: string; size: number }>> {
      ...
      for (const { key, type, ext } of mediaTypes) {
        ...
        try {
          ...
          log.info('Media downloaded for inbox staging', { type, filename, size: buffer.length });
        } catch (err) {
    ...
    async function sendRawMessage(jid: string, text: string): Promise<string | undefined> {
      const taggedText = tagWhatsAppReply(text);
      try {
        assertCanSendWhatsAppMessage(privateSafety, jid);
      } catch (err) {
        if (err instanceof WhatsAppSafetyError) {
          logSafetyBlock('outbound', err.reason, { targetJid: jid });
        }
      ...
      if (!connected) {
        outgoingQueue.push({ jid, text: taggedText });
        log.info('WA disconnected, message queued', { jid, queueSize: outgoingQueue.length });
        return;
      }
      try {
        const sent = await guardedSendMessage(jid, { text: taggedText });
        if (sent?.key?.id && sent.message) {
          sentMessageCache.set(sent.key.id, sent.message);
        ...
        if (qr && !phoneNumber) {
          ...
          (async () => {
            try {
              ...
              await QRCode.toFile(qrPngFile, qr, { type: 'png', width: 512, margin: 2 });
              fs.writeFileSync(qrTextFile, qr, 'utf-8');
              log.info('WhatsApp QR code saved for scanning', { path: qrPngFile });
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // Resolve sender before touching content or media so rejected chats
            // never have attachments downloaded or message bodies inspected.
            ...
            const safetyDecision = evaluateWhatsAppInbound(privateSafety, {
              remoteJid: rawJid,
              chatJid,
              senderJid: sender,
              fromMe,
            ...
            if (!safetyDecision.allowed) {
              logSafetyBlock('inbound', safetyDecision.reason, { remoteJid: rawJid, chatJid, senderJid: sender });
              continue;
            }
            ...
            // In private self-chat mode, user-typed messages and bot-sent
            // echoes both arrive as fromMe=true. The sent cache is the
            // boundary: process user-typed self-chat entries, drop our own.
            ...
            if (!privateSafety.enabled && fromMe) {
              const isSelfChat = botPhoneJid && chatJid === botPhoneJid;
              const isWhitelistedFromMeChat = fromMeChatAllowlist.has(chatJid);
              if (!isSelfChat && !isWhitelistedFromMeChat) continue;
    ...
    const adapter: ChannelAdapter = {
      ...
      async deliver(
        ...
        if (content.type === 'ask_question' && content.questionId && content.options) {
          ...
          const text = formatOutgoingWhatsAppText(`*${title}*\n\n${question}\n\nReply with:\n${optionLines}`);
        ...
        if (content.operation === 'reaction' && content.messageId && content.emoji) {
          try {
            await guardedSendMessage(platformId, {
              react: {
        ...
        if (hasFiles) {
          ...
          const formattedCaption = text ? formatOutgoingWhatsAppText(text) : undefined;
          for (const file of message.files!) {
            try {
              ...
              const sent = await guardedSendMessage(platformId, mediaMsg);
              if (sent?.key?.id && sent.message) {
                sentMessageCache.set(sent.key.id, sent.message);
              }
        ...
        if (text) {
          return sendRawMessage(platformId, formatOutgoingWhatsAppText(text));
        }
      },

      async setTyping(platformId: string) {
        try {
          assertCanSendWhatsAppMessage(privateSafety, platformId);
        } catch (err) {
          if (err instanceof WhatsAppSafetyError) {
            logSafetyBlock('typing', err.reason, { targetJid: platformId });
            return;
          }
          throw err;
        }
      ...
      async syncConversations(): Promise<ConversationInfo[]> {
        if (privateSafety.enabled) {
          return privateSafety.allowedJid
            ? [{ platformId: privateSafety.allowedJid, name: 'Allowlisted WhatsApp chat', isGroup: false }]
            : [];
        }
        try {
          const groups = await sock.groupFetchAllParticipating();
