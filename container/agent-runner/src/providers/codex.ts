...
export class CodexProvider implements AgentProvider {
  ...
  constructor(options: ProviderOptions = {}) {
    ...
    this.effort =
      options.effort ??
      (options.env?.CODEX_MODEL_REASONING_EFFORT as string | undefined) ??
      (options.env?.CODEX_EFFORT as string | undefined);
  }
  ...
  query(input: QueryInput): AgentQuery {
    ...
    const self = this;
    const model = input.model ?? self.model;
    const effort = input.effort ?? self.effort;
...
async function* runOneTurn(
  ...
  try {
    ...
    await startCodexTurn(server, { threadId, inputText, model, effort, cwd });
